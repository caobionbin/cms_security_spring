...
<script type="text/javascript"
	src="<s:url value='/js/jstree/jquery.jstree.js'/>"></script>
...
<script type="text/javascript">
	$(function() {
		...
		$("#menuTree").jstree({
			"json_data" : {
				"ajax" : {
					"type" : "POST",
					"url" : "<s:url value='/menu/listtree.do'/>"
				}
			},
			"themes" : {
				"icons" : false
					...
					if (tempid != "") {
						for (var i = 0; i < initMenus.length; i++) {
							...
							if (tempid == initMenus[i]) {
								...
								$(this).removeClass("jstree-closed").addClass(
										"jstree-open");
							}
						}
					}
	...
	function getSelect() {
		...
		// 			alert($("#menuTree").jstree("get_text", $(n)) + "/"
		// 					+ $(n).attr("id"));
		// 		});
		var menuIds =  "";
...
<body>
	...
	<div class="rounded table">
		...
		<form id="form1" action="<s:url value='/role/saveRoleResource.do'/>"
			method="post" onsubmit="">
			<table class="update" cellpadding="0" cellspacing="1" border="0">
				<tbody>
					<tr>
						<td>角色名称： <span style="color: blue;">${roleEntity.roleName }</span>
							<input type="hidden" name="roleId" value="${roleEntity.id}" /> <input
							type="hidden" name="menuIds" value="${menuIds}" />
						</td>
					...
					<tr class="even">
						<td><input name="allChkbox" id="allChkbox" type="checkbox"
							onclick="selectAllNode(this);" /> 全选/取消</td>
