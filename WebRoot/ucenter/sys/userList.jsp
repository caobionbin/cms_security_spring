...
<script type="text/javascript">
    ...
    function add(){
    	$.jBox("iframe:<s:url value='/user/viewUser.do'/>?id=0", {
    	    title: "添加用户",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function edit(id){
    	$.jBox("iframe:<s:url value='/user/viewUser.do'/>?id="+id, {
    	    title: "修改用户",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function changeUserStatus(id,status){
    	...
    	$.jBox.confirm("确定 "+(status==0?"禁用":"启用")+" 用户吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/user/frozen.do'/>",{id:id,state:status},function(responseText){
    ...
    function delUser(id,msg){
    	...
    	$.jBox.confirm("确定删除 ["+msg+"] 用户吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/user/delUser.do'/>",{id:id},function(responseText){
    	    		if(responseText==true){
    	    			$.jBox.info("操作成功，请刷新查看结果", "成功信息",{top: '20%'});
    	    			window.location.reload();
    	    		}else{
    ...
    function userRoleSet(userId){
    	$.jBox("iframe:<s:url value='/user/userRoleSet.do'/>?userId="+userId, {
    	    title: "用户授权",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
			...
			<form action="" method="post" name="manage" id="manage">
				<table class="list td_align" cellpadding="0" cellspacing="1"
					...
					<c:forEach items="${dataList}" var="dataItem" varStatus="rowStatus">
						...
						<td><c:choose>
								<c:when test="${dataItem.state==0}">
									<font color='red'>禁用</font>
								</c:when>
								<c:when test="${dataItem.state==1}">
									<font color='green'>启用</font>
								</c:when>
								<c:otherwise>
									<font color='gray'>未知:${dataItem.state}</font>
								</c:otherwise>
							</c:choose></td>
						<td><fmt:formatDate value="${dataItem.insertTime}"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td><fmt:formatDate value="${dataItem.lastLogintime}"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td class="operation"><a href="javascript:;"
							onclick="edit(${dataItem.id});"> <img
								...
								<c:when test="${dataItem.state==0}">
									<a href="javascript:;"
										onclick="changeUserStatus(${dataItem.id},1);"> <img
										src="<s:url value='/css/images/operation/lock_unlock.png'/>"
										title="启用用户" />
									</a>
								</c:when>
								<c:otherwise>
									<a href="javascript:;"
										onclick="changeUserStatus(${dataItem.id},0);"> <img
										src="<s:url value='/css/images/operation/lock.png'/>"
										title="禁用用户" />
									</a>
								</c:otherwise>
							</c:choose> <a href="javascript:;" onclick="userRoleSet(${dataItem.id});">
								<img src="<s:url value='/css/images/operation/member.png'/>"
								title="用户权限" />
						</a> <a href="javascript:;"
							onclick="delUser(${dataItem.id},'${dataItem.loginName}');"> <img
