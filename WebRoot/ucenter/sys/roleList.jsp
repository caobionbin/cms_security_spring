...
<script type="text/javascript">
    ...
    function add(){
    	$.jBox("iframe:<s:url value='/role/view.do'/>?id=0", {
    	    title: "添加角色",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function edit(id){
    	$.jBox("iframe:<s:url value='/role/view.do'/>?id="+id, {
    	    title: "修改角色",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function delRole(id,msg){
    	...
    	$.jBox.confirm("确定删除 ["+msg+"] 角色吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/role/delete.do'/>",{id:id},function(responseText){
    ...
    function changeRoleStatus(id,status){
    	...
    	$.jBox.confirm("确定 "+(status==0?"禁用":"启用")+" 角色吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	
    	    	$.post("<s:url value='/role/updateState.do'/>",{id:id,state:status},function(responseText){
    	    		if(responseText==true){
    	    			$.jBox.info("操作成功，请刷新查看结果", "成功信息",{top: '20%'});
    	    			parent.location.reload();
    	    		}else{
    ...
    function roleResourceSet(roleId){
    	$.jBox("iframe:<s:url value='/role/roleResourceSet.do'/>?roleId="+roleId, {
    	    title: "角色资源授权",
    	    width: 700,
    	    height: 550,
    	    buttons: {}
					...
					<c:forEach items="${dataList}" var="dataItem" varStatus="rowStatus">
						...
						<td class="operation"><a href="javascript:;"
							onclick="edit(${dataItem.id});"> <img
								...
								<c:when test="${dataItem.state==0}">
									<a href="javascript:;"
										onclick="changeRoleStatus(${dataItem.id},1);"> <img
								...
								<c:otherwise>
									<a href="javascript:;"
										onclick="changeRoleStatus(${dataItem.id},0);"> <img
							...
							onclick="roleResourceSet(${dataItem.id});"> <img
								src="<s:url value='/css/images/operation/folder.png'/>"
								title="角色权限" />
						</a> <a href="javascript:;"
